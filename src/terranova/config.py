"""Per-user Terranova settings — distinct from per-project state and from
...
class Settings:
    """Global per-user Terranova preferences."""
...
def _path() -> Path:
    try:
        ...
        return Path(platformdirs.user_config_dir("terranova")) / "settings.json"
    except ImportError:  # pragma: no cover
        return Path.home() / ".config" / "terranova" / "settings.json"

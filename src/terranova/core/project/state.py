...
that Terranova persists alongside the QGIS project file as ``terranova.json``.
...
class ProjectState(BaseModel):
    """The whole Terranova project, persisted as ``terranova.json``."""
    ...
    def load(cls, path: Path) -> "ProjectState":
        """Load + validate a ``terranova.json``.  Returns a fresh state if absent.
...
class ProjectStateMigrationError(RuntimeError):
    """Raised when a ``terranova.json`` from an unknown future version is loaded."""
...
def migrate(raw: dict) -> dict:
    ...
    if version > SCHEMA_VERSION:
        raise ProjectStateMigrationError(
            f"terranova.json schema_version={version} is newer than this "
            f"plugin supports (max {SCHEMA_VERSION}).  Please upgrade Terranova."
        )

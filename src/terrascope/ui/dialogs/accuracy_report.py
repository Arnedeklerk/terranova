...
class AccuracyReportDialog(QDialog):
    ...
    def __init__(self, iface: "QgisInterface", parent: QWidget | None = None) -> None:
        ...
        self.setWindowTitle("TerraScope — Accuracy report")
        self.resize(560, 320)
    ...
    def _pick_output(self) -> None:
        default = str(Path.home() / "terrascope_accuracy.pdf")
    ...
    def _on_done(self) -> None:
        ...
        if self._task and self._task.report_summary:
            ...
            self.iface.messageBar().pushSuccess(
                "TerraScope", f"Accuracy PDF written to {self._task.out_pdf.name}"
            )
...
class _AccuracyTask(QgsTask):
    ...
    def __init__(
        ...
        super().__init__(f"TerraScope: accuracy {raster_path.name}", QgsTask.CanCancel)
    ...
    def run(self) -> bool:
        try:
            ...
            render_pdf(report, self.out_pdf, title="TerraScope — Accuracy report")
            self._emit(100, "Done.")
            return True
        except Exception as exc:
            self.error_text = f"{type(exc).__name__}: {exc}"
            QgsMessageLog.logMessage(
                f"Accuracy report failed: {exc!r}", "TerraScope", Qgis.MessageLevel.Critical
            )
            return False

...
class SamDialog(QDialog):
    ...
    def __init__(self, iface: "QgisInterface", parent: QWidget | None = None) -> None:
        ...
        self.setWindowTitle("TerraScope — Segment with SAM")
        self.resize(620, 380)
    ...
    def _pick_output(self) -> None:
        default = str(Path.home() / "terrascope_sam.gpkg")
    ...
    def _start_picking(self) -> None:
        ...
        self.iface.messageBar().pushInfo(
            "TerraScope", "Click foreground points on the map. Press Escape when done."
        )
    ...
    def _on_done(self) -> None:
        ...
        if self._task and self._task.result_path:
            ...
            self.iface.messageBar().pushSuccess(
                "TerraScope", f"Segmented {self._task.result_path.name}"
            )
...
class _SamTask(QgsTask):
    ...
    def __init__(
        ...
        super().__init__(f"TerraScope: SAM {raster_path.name}", QgsTask.CanCancel)
        self.mode = mode

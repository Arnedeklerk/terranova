...
Wraps :func:`terrascope.core.ml.foundation.finetune` with a Qt form.  The
...
class FoundationDialog(QDialog):
    ...
    def __init__(self, iface: "QgisInterface", parent: QWidget | None = None) -> None:
        ...
        self.setWindowTitle("TerraScope — Fine-tune foundation model")
        self.resize(680, 480)
        self._build_ui()
    ...
    def _on_done(self) -> None:
        ...
        if self._task and self._task.checkpoint_path:
            ...
            self.iface.messageBar().pushSuccess(
                "TerraScope", f"Fine-tuned backbone saved to {self._task.checkpoint_path.parent}"
            )
...
class _FoundationTask(QgsTask):
    ...
    def __init__(
        ...
        super().__init__(f"TerraScope: fine-tune {backbone}", QgsTask.CanCancel)

"""TerraScope — modern semi-automatic classification for QGIS.

This module exposes the QGIS plugin entry-point, ``classFactory``.  All heavier
imports (Qt, qfluentwidgets, QtWebEngine) are deferred to :mod:`terrascope.plugin`
...
def classFactory(iface):  # type: ignore[no-untyped-def]
    ...
    """
    # Deferred import so that ``import terrascope`` from non-QGIS contexts
    # (CI, headless tests of the ``core`` layer) does not fail.
    from .plugin import TerraScopePlugin

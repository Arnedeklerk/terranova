...
`terranova.api` is the stable public surface for scripting Terranova from notebooks or the QGIS Python console. Everything reachable from this module is covered by semantic versioning.
...
| Symbol | Description |
|--------|-------------|
| `api.__version__` | Current Terranova version. |
| `api.ProjectState` | Pydantic model for the project state file (`terranova.json`). |
...
Internal modules (`terranova.core.*`, `terranova.controllers.*`) may change between minor versions; `terranova.api` may not.
